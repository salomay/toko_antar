...
exports.auth =  function(req, res) {
    ...
    if(error){
        ...
        const rememberToken= jwt.sign({userId,name,email},process.env.REMEMBER_TOKEN_SECRET,{
            expiresIn:'1d'
        });
...
exports.me = function(req, res) {
    ...
    if(req.query.email){
        ...
        connection.query("SELECT * FROM users a LEFT JOIN user_profiles b ON a.id=b.customer_id LEFT JOIN address c ON a.id=c.customer_id  WHERE a.email=? ",
            [email], function (error, rows, fields){
            if(error){
                console.log(error);
                response.err(res)
            } else{
...
exports.orders = function(req, res) {
    ...
    "",
        [search,shop_id,page,limit,orderBy,sortedBy], function (error, rows, fields){
        if(error){
            console.log(error);
            response.err(res)
        } else{
...
exports.popular_products = function(req, res) {
    ...
    connection.query(" SELECT * FROM products a "+
    "  JOIN types b ON a.type_id = b.id "+
    "  JOIN shops c ON a.shop_id=c.id "+
    "  JOIN category_product d ON a.id=d.product_id "+
    "  JOIN categories e ON d.category_id=e.id "+
    "  JOIN product_tag f ON a.id=f.product_id "+
    "  JOIN tags g ON f.tag_id=g.id "+
    "",
        [], function (error, rows, fields){
        if(error){
            console.log(error);
...
exports.userUpdate = async function(req, res) {
    ...
    if(req.body.id){
        ...
        " where id=? "+
        "",
            [name,email,id_kel,no_telp,id], function (error, rows, fields){
            if(error){
                console.log(error);
                response.err(res)
            } else{
...
exports.kabupaten = function(req, res) {
    ...
    if(id_prov){
        ...
        "",
            [id_prov], function (error, rows, fields){
            if(error){
                console.log(error);
                response.err(res)
            } else{
...
exports.kecamatan = function(req, res) {
      ...
      if(id_kab){
        ...
        "",
            [id_kab], function (error, rows, fields){
            if(error){
                console.log(error);
                response.err(res)
            } else{
...
exports.kelurahan = function(req, res) {
    ...
    if(id_kec){
      ...
      "",
          [id_kec], function (error, rows, fields){
          if(error){
              console.log(error);
              response.err(res)
          } else{
    ...
    if(req.query.id){
        ...
        connection.beginTransaction(function(err) {
            if (err) { throw err; }
                ...
                connection.query("SELECT DISTINCT a.id ,a.name, a.slug, a.description, a.in_stock, a.is_taxable, a.price, a.sale_price, a.unit, a.status, a.image, a.type_id, a.in_stock, a.width, a.height, a.length, d.name as type_name, "+
                ...
                " WHERE a.id = ? ",
                    [req.query.id], function (error, rows, fields){
                    if(error){
                        console.log(error);
                        response.err(res);
                    }else{
                        ...
                        " WHERE product_id = ? ",
                            [rows[0].id], function (error, data_product_tag, fields){
                            if(error){
                                console.log(error);
                                response.err(res);
                            } else{
                                ...
                                " WHERE a.product_id=? ",
                                    [rows[0].id], function (error, data_attribute_value_list, fields){
                                    if(error){
                                        console.log(error);
                                        response.err(res);
                                    }else{
                                        ...
                                        " WHERE product_id = ?  ",
                                            [rows[0].id], function (error, data_variation_options, fields){
                                            if(error){
                                                console.log(error);
                                                response.err(res);
                                            } else{
                                                ...
                                                var array_variation=[];
                                                    ...
                                                    " JOIN attributes c ON b.attribute_id = c.id "+
                                                    " WHERE a.product_id = ?  ",
                                                        [rows[0].id], function (error, data_attribute, fields){
                                                        if(error){
                                                            console.log(error);
                                                            response.err(res);
                                                        } else{
                                                                ...
                                                                var data_={
                                                                    ...
                                                                    "attribute": {
                                                                        ...
                                                                        "values":array_attribute_value_list,
...
exports.products_input = function(req, res) {
    ...
    connection.beginTransaction(function(err) {
        ...
        " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,now(),now(),now()) ",
            [name.charAt(0).toUpperCase() + name.slice(1), name.toLowerCase(), description, type_id, price, sale_price, quantity, 1, status, unit, height, width, length, images ], function (error, result, fields){
            if(error){
                ...
                connection.query(" INSERT INTO category_product (product_id,category_id) VALUES (?,?) ",
                    [result.insertId, categories], function (error, rows, fields){
                    if(error){
...
exports.categories = function(req, res) {
    ...
    if(req.query.id){
        ...
        "",
            [id_sub_categories], function (error, rows, fields){
            if(error){
                console.log(error);
                response.err(res);
            } else{
        ...
        "",
            [type_id], function (error, rows, fields){
            if(error){
                console.log(error);
                response.err(res);
            } else{
...
exports.categories_input = function(req, res) {
    ...
    var images=JSON.stringify(image);
        ...
        " VALUES (?,?,?,?,?,?,?,now(),now(),now()) ",
            [name.charAt(0).toUpperCase() + name.slice(1), name.toLowerCase(), icon,images,details, type_id, parent ], function (error, rows, fields){
            if(error){
...
exports.categories_update = function(req, res) {
    ...
    var images=JSON.stringify(image);
        ...
        "  ",
            [name.charAt(0).toUpperCase() + name.slice(1), name.toLowerCase(), icon,images,details, type_id, parent,req.params.id ], function (error, rows, fields){
            if(error){
...
exports.taxes_input = function(req, res) {
    ...
    var rate = req.body.rate;    
        ...
        " VALUES (?,?,now(),now()) ",
            [name, rate,], function (error, rows, fields){
            if(error){
...
exports.taxes_update = function(req, res) {
    ...
    var rate = req.body.rate;    
        ...
        " WHERE id=? ",
            [name,rate,req.params.id ], function (error, rows, fields){
            if(error){
...
exports.coupons_input = function(req, res) {
    ...
    var images=JSON.stringify(image);
        ...
        " VALUES (?,?,?,?,?,?,?,now(),now(),now()) ",
            [code, description, images, type, amount, active_from, expire_at ], function (error, rows, fields){
            if(error){
...
exports.coupons_update = function(req, res) {
    ...
    var images=JSON.stringify(image);
        ...
        " WHERE id=? ",
            [code, description, images, type, amount, active_from, expire_at,req.params.id ], function (error, rows, fields){
            if(error){
...
exports.attributes = function(req, res) {
    ...
    var query ="";
        ...
        " WHERE a.product_id='"+req.query.product_id+"' ";
            ...
            query = "SELECT a.id, a.slug, a.name, a.created_at, a.updated_at, b.id as id_ , b.attribute_id, b.value, b.meta, b.created_at as created_at_, b.updated_at as updated_at_ FROM attributes a JOIN attribute_values b ON a.id=b.attribute_id  order by "+orderBy+" "+sortedBy+" ";
...
exports.attributes_input = function(req, res) {
    ...
    connection.beginTransaction(function(err) {
        ...
        "",
            [name.charAt(0).toUpperCase() + name.slice(1), name.toLowerCase()], function (error, rows, fields){
            if(error){
                connection.rollback(function() {
                    throw error;
                  });
...
exports.attributes_update = function(req, res) {
    ...
    connection.beginTransaction(function(err) {
        ...
        "",
            [name.charAt(0).toUpperCase() + name.slice(1), name.toLowerCase(),req.params.id], function (error, rows, fields){
            if(error){
                connection.rollback(function() {
                    throw error;
                  });
                ...
                "",
                    [req.params.id], function (error, rows2, fields){
                    if(error){
                        connection.rollback(function() {
                            throw error;
                          });
...
exports.types_input = function(req, res) {
    ...
    console.log(req.body)
        ...
        "",
            [name.charAt(0).toUpperCase() + name.slice(1), name.toLowerCase(),icon ], function (error, rows, fields){
            if(error){
...
exports.types_update = function(req, res) {
    ...
    var icon = req.body.icon;    
        ...
        "",
            [name.charAt(0).toUpperCase() + name.slice(1), name.toLowerCase(),icon,req.params.id], function (error, rows, fields){
            if(error){
                connection.rollback(function() {
                    throw error;
                  });
...
exports.tags_input = function(req, res) {
    ...
    var type_id = req.body.type_id;
        ...
        " VALUES (?,?,?,?,?,?,now(),now(),now()) ",
            [name.charAt(0).toUpperCase() + name.slice(1), name.toLowerCase(), icon, JSON.stringify(image),details, type_id ], function (error, rows, fields){
            if(error){
...
exports.tags_update = function(req, res) {
    ...
    var type_id = req.body.type_id;
        ...
        " WHERE id=? ",
            [name.charAt(0).toUpperCase() + name.slice(1), name.toLowerCase(), icon, JSON.stringify(image),details, type_id, id ], function (error, rows, fields){
            if(error){
...
exports.reportHarian = function(req, res) {
    ...
    " ORDER BY a.date_create desc ", 
        [id_outlet,bulan,tahun], function (error, rows, fields){
        if(error){
            console.log(error);
            response.err(res)
        } else{
...
exports.laporanBulananKe = function(req, res) {
    ...
    "GROUP BY YEAR(date_create), MONTH(date_create),DATE(date_create)  ",
        [id_outlet,bulan], function (error, rows, fields){
        if(error){
            console.log(error);
            response.err(res)
        } else{
...
exports.laporanHarianKe = function(req, res) {
    ...
    "GROUP BY YEAR(date_create), MONTH(date_create),DAY(date_create),id_order  ",
        [id_outlet,bulan,tgl], function (error, rows, fields){
        if(error){
            console.log(error);
            response.err(res)
        } else{
...
exports.laporanNoOrder = function(req, res) {
    ...
    " where a.id_outlet=? and a.id_order=? ",
        [id_outlet,id_order], function (error, rows, fields){
        if(error){
            console.log(error);
            response.err(res)
        } else{
