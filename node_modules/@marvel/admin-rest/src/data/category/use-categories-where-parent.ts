import {
  ...
  const fetchParentCategories = async ({queryKey,}: QueryParamsType) => {
    ...
    const url = `${API_ENDPOINTS.CATEGORIES}?type_id=${params.text}`;
  ...
  const useCategoriesWhereParentQuery = (options: CategoriesQueryOptionsType) => {
  ...
  export { useCategoriesWhereParentQuery, fetchParentCategories };
